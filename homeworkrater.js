...
if (Meteor.isClient) {
  ...
  Template.results.helpers({
    ...
    selectedPageHomeworkAll: function() {
      return Homework.find({
        ...
        class: Session.get('currentClass')
      });
    ...
    flaggedProblemsText: function() {
      ...
      var currentPageHomework = Homework.find({
        page: Session.get('currentPage'),
        class: Session.get('currentClass')
      }); //get HW from database with current page
      ...
      if (currentPageHomework) {
        currentPageHomework.forEach(function(hw) {
          ...
          selectedProblems = _.sortBy(selectedProblems, function(e) {
            return e
          }); //sort problems in ascending order
        })
    ...
    flaggedProblems: function() {
      ...
      var currentPageHomework = Homework.find({
        page: Session.get('currentPage'),
        class: Session.get('currentClass')
      }).fetch(); //get HW from database with current page
      var selectedProblems = [];
  ...
  Template.students.events({
    'change #studentList': function(event) {
      console.log("Changed");
      Session.set('selectedStudent', $("#studentList").val());
...
problemsStringToArray = function(string) {
  ...
  if (typeof(string) == "string") {
    ...
    problemString.forEach(function(e) {
      blankArray.push(parseInt(e))
    }); //this adds the problems in the string array as ints

    return _.sortBy(blankArray, function(e) {
      return e
